...
{
    // Smartcoin: Never mine witness tx
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        while (nMaxTries > 0 && pblock->nNonce < nInnerLoopCount && !CheckProofOfWork(pblock->GetHash(), pblock->nBits, Params().GetConsensus(nHeight))) {
            ++pblock->nNonce;
            --nMaxTries;
        }
        if (nMaxTries == 0) {
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Smartcoin is not connected!");
