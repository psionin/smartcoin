...
const std::string strMessageMagic = "Smartcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetSmartcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Smartcoin: Switch maturity at depth 145,000
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Smartcoin: BIP30 has been active since inception
    ...
    // Smartcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetSmartcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(), block.nBits, Params().GetConsensus(0)))
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Smartcoin: We probably want to disable this
    int nLockTimeFlags = 0;
    ...
    if (nHeight >= consensusParams.BIP34Height)
    {
        ...
        if (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
            ...
            //LogPrintf("AcceptBlock: Received block %s was rejected: %s. Block info: [height=%d, size=%u, tx=%u, time=%u]\n",
            //    block.GetHash().ToString(), FormatStateMessage(state),
            //    pindexPrev->nHeight + 1, ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION), block.vtx.size(), block.GetBlockTime());
