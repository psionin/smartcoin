...
class WalletAccountsTest(BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        assert_equal(node.getbalance(), 400085)
        ...
        for account in accounts:
            ...
            assert_equal(node.getreceivedbyaccount(account), 20)
            node.move(account, "", node.getbalance(account))
        
        node.generate(101)
        
        expected_account_balances = {"": 400188}
        for account in accounts:
            expected_account_balances[account] = 0
        
        assert_equal(node.listaccounts(), expected_account_balances)
        
        assert_equal(node.getbalance(""), 400188)
        ...
        for account in accounts:
            ...
            node.sendfrom("", multisig_address, 50000)
        
        node.generate(101)
        
        for account in accounts:
            assert_equal(node.getbalance(account), 50000)
